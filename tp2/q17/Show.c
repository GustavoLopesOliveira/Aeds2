...
void heapfy(Show array[], int tam, int i, int comparacoes[], int movimentacoes[]){
    int maior = i;
    int esq = (2*i)+1;
    int dir = (2*i)+2;
    ...
    if (esq < tam) {
        comparacoes[0]++;
        int comp = strcasecmp(array[esq].director, array[maior].director);
        if (comp > 0 || (comp == 0 && strcasecmp(array[esq].title, array[maior].title) > 0)) {
            maior = esq;
        }
    }
    if(dir < tam){
        comparacoes[0]++;
        int comp = strcasecmp(array[dir].director, array[maior].director);
        if (comp > 0 || (comp == 0 && strcasecmp(array[dir].title, array[maior].title) > 0)) {
            maior = dir;
        }
    }
    if(maior != i){
        swap(array, maior, i);
        movimentacoes[0] += 3;
        heapfy(array, tam, maior, comparacoes, movimentacoes);
    }
}

void heapsortParcial(Show array[], int tam, int k, int comparacoes[], int movimentacoes[]){
    for(int i = (tam/2)-1; i >= 0; i--){
        heapfy(array, tam, i, comparacoes, movimentacoes);
    }
    
    for(int i = k+1; i <= tam; i++){
        if(array[i].director < array[1].director || (array[i].director == array[1].director && array[i].title < array[1].title)){
    ...
    for(int i = tam-1; i > 0; i--){
        swap(array, 0, i);
        movimentacoes[0] += 3;
        heapfy(array, i, 0, comparacoes, movimentacoes);
    }
}

void arquivoLog(int matricula, double duracao, int comparacoes[], int movimentacoes[]) {
	FILE *arq = fopen("869118_heapsortParcial.txt", "w");
	if(arq) {
...
int main() {
	...
	heapsortParcial(listaIds, contIds, 10, comparacoes, movimentacoes);
	for(int i = 0; i < 10; i++){
	    imprimir(listaIds[i]);
	}
	free(show);
    ...
    arquivoLog(869118, duracao, comparacoes, movimentacoes);
	return 0;
}