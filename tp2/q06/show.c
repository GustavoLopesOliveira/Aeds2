...
void ordenacaoPorSelecaoRecursivo(Show lista[], int pos, int tam, int comparacoes[], int movimentacoes[]){
    if(pos >= tam-1){
        return;
    }
    ...
    for(int j = (pos+1); j < tam; j++){
        comparacoes[0]++;
        if(strcasecmp(lista[menor].title, lista[j].title) > 0){
            menor = j;
        }
    }
    
    Show temp = lista[menor];
    lista[menor] = lista[pos];
    lista[pos] = temp;
    
    movimentacoes[0] += 3;
    
    ordenacaoPorSelecaoRecursivo(lista, pos+1, tam, comparacoes, movimentacoes);
}

void arquivoLog(int matricula, double duracao, int comparacoes[], int movimentacoes[]) {
	FILE *arq = fopen("869118_selecaoRecursiva.txt", "w");
	if(arq) {
...
int main() {
	...
	ordenacaoPorSelecaoRecursivo(listaIds, 0, contIds, comparacoes, movimentacoes);
