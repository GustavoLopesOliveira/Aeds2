...
void arquivoLog(int matricula, double duracao, int comparacoes[], int movimentacoes[]) {
	FILE *arq = fopen("869118_radixsort.txt", "w");
	if(arq) {
...
/*
O Radix Sort é um algoritmo de ordenação que organiza números processando dígito por dígito,
 da direita para a esquerda. ELe utiliza o Counting Sort que é estável.
 
 O(n*k) onde n é o número de elementos e k é o número de dígitos do maior número.
