...
void ordenacaoPorInsercaoParcial(Show lista[], int tam, int k, int comparacoes[], int movimentacoes[]){
    Show temp;
    for(int i = 1; i < tam; i++){
        temp = lista[i];
        int j = (i < k) ? i-1 : k-1;
        while(j>=0){
            int comp = strcasecmp(lista[j].type, temp.type);
            if(comp > 0 || (comp == 0 && (strcasecmp(lista[j].title, temp.title) > 0))){
                lista[j+1] = lista[j];
                j--;
                movimentacoes[0]++;
                comparacoes[0]++;
...
void arquivoLog(int matricula, double duracao, int comparacoes[], int movimentacoes[]) {
	FILE *arq = fopen("869118_insercaoparcial.txt", "w");
	if(arq) {
...
int main() {
	...
	ordenacaoPorInsercaoParcial(listaIds, contIds, 10, comparacoes, movimentacoes);
	for(int i = 0; i < 10; i++){
	    imprimir(listaIds[i]);
	}
	free(show);
