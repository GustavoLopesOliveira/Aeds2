...
void insercaoPorCor(Show array[], int tam, int cor, int h, int comparacoes[], int movimentacoes[]){
    for(int i = h+cor; i < tam; i+=h){
        Show temp = array[i];
        int j = i-h;
        
        while(j>=0){
            int comp = strcasecmp(array[j].type, temp.type);
            comparacoes[0]++;
            if(comp > 0 || (comp == 0 && strcasecmp(array[j].title, temp.title) > 0)){
                array[j+h] = array[j];
                j -= h;
                movimentacoes[0]++;
            }else{
...
void shellSort(Show array[], int tam, int comparacoes[], int movimentacoes[]){
    int h = 1;
    while(h < tam){
        h = (h*3)+1;  
    }
    while(h > 0){
        ...
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(array, tam, cor, h, comparacoes, movimentacoes);
        } 
    }
}

void arquivoLog(int matricula, double duracao, int comparacoes[], int movimentacoes[]) {
	FILE *arq = fopen("869118_shellsort.txt", "w");
	if(arq) {
...
int main() {
	...
	shellSort(listaIds, contIds, comparacoes, movimentacoes);
